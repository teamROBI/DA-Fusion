...
# Code adapted from: https://github.com/davisvideochallenge/davis-2017/blob/master/python/lib/davis/measures/f_boundary.py
def boundary_overlap(i, j, predicted_mask, gt_mask, rgb, our_input, bound_th=0.003, save_image=False):
    """
    ...
    gt_boundary = seg2bmap(gt_mask.copy());
    ...
    if save_image:
        # vis_img = np.hstack([rgb, our_input])
        vis_boundary = np.hstack([fg_boundary*255, gt_boundary*255])
        vis_mask =  np.hstack([predicted_mask*255, gt_mask*255])
        vis_dil = np.hstack([fg_dil*255, gt_dil*255])
        vis_all_img = np.vstack([vis_boundary, vis_mask, vis_dil])
        vis_all_img = np.array(vis_all_img, dtype=np.uint8)

        from ..paths import DATASET_ROOT as EVAL_RESULT_ROOT
        _img_dir = os.path.join(EVAL_RESULT_ROOT, "image_result")
        os.makedirs(_img_dir, exist_ok=True)
        cv2.imwrite(os.path.join(_img_dir, "boundary_overlap_{}_{}.png".format(j, i)), vis_all_img)
...
def multilabel_metrics(prediction, gt, rgb, our_input, obj_detect_threshold=0.75, return_assign=False, save_img=False):
    ...
    for i, gt_i in enumerate(labels_gt):
        ...
        for j, pred_j in enumerate(labels_pred):
            ...
            ### Boundary Stuff ###
            boundary_stuff[i,j] = boundary_overlap(i, j, pred_j_mask, gt_i_mask, rgb, our_input, save_image=save_img)
